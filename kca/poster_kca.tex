...
% compact pipeline styles (4-layer refinement flow)
\tikzset{
  pipe/.style={rectangle, rounded corners, minimum height=15mm, text width=150mm,
    align=center, draw=kcanavy, line width=0.8pt, fill=kcanavy!7,
    font=\fontsize{16}{19}\selectfont, inner sep=3mm},
  pipehi/.style={pipe, fill=orange!20},
  parr/.style={-{Stealth[length=4mm]}, line width=1.1pt, draw=kcanavy},
}
...
% III. 연구방법
\begin{textblock}{345}(50,932)
  \fontsize{28}{37}\selectfont\justifying
  자료는 X 공개 한국어 글에서 모았다(사용자명$\cdot$외부 식별자 자동 비식별). 질의는 \textbf{정밀$\cdot$확장$\cdot$맥락 점검$\cdot$누적 확장}의 네 층으로 설계했고, 회수 게시물은 2010년 9월$\sim$2026년 4월(약 15년 7개월)에 걸쳐 있다. 아래 \textbf{LLM 보조 4층 정제 절차}로 원자료를 좁혀 본문 코퍼스를 확정했다.
  \par\vspace{3mm}
  \begin{minipage}[t]{166mm}
    \centering\vspace{0pt}
    \begin{tikzpicture}[node distance=5mm]
      \node[pipe](a){\textbf{① 사전 오염 제거}\\[0.2ex]{\fontsize{14}{17}\selectfont 광고$\cdot$기관 홍보$\cdot$봇$\cdot$비심리 상담 제거}};
      \node[pipe,below=of a](b){\textbf{② 규칙 기반 1차 분류}\\[0.2ex]{\fontsize{14}{17}\selectfont 자기/가족 주어 $+$ 상담 동사 $+$ 정신건강 어휘 $\rightarrow$ 유지/제외/검토}};
      \node[pipehi,below=of b](c){\textbf{③ LLM 판정}\\[0.2ex]{\fontsize{14}{17}\selectfont 실제 경험 여부$\cdot$시점$\cdot$장면$\cdot$평가 방향$\cdot$신뢰도 산출}};
      \node[pipe,below=of c](d){\textbf{④ 보수적 본문 확정}\\[0.2ex]{\fontsize{14}{17}\selectfont 정신과$\cdot$강한 약물$\cdot$관찰자 시점 제외}};
      \draw[parr](a)--(b);\draw[parr](b)--(c);\draw[parr](c)--(d);
    \end{tikzpicture}
    \par\vspace{1mm}{\fontsize{17}{21}\selectfont\textit{그림 1.} LLM 보조 4층 정제 절차.}
  \end{minipage}\hfill
  \begin{minipage}[t]{172mm}
    \centering\vspace{0pt}
    \includegraphics[width=172mm]{../figures/corpus_funnel.pdf}
    \par\vspace{1mm}{\fontsize{17}{21}\selectfont\textit{그림 2.} 원자료 3{,}652건 $\rightarrow$ 본문 코퍼스 361건(9.9\%). 관련성 유지본 479$\cdot$엄격 확정본 451건 경유.}
  \end{minipage}
\end{textblock}
