...
set_y("그룹 39", 440); set_y("Picture 11", 545); set_y("Picture 15", 755)
center_label("TextBox 8", 179.8, L); center_label("TextBox 12", 545, L); center_label("TextBox 16", 755, L)
...
"직접 분석한 연구는 아직 드물다.")
sp = find("연구의 필요성을 작성"); setbox(sp, 58, 252, 345, 180); fill_para(sp, [seoron], 23)

sp = find("연구목적과 연구문제를 작성"); setbox(sp, 61, 467, 345, 70)
fill_para(sp, [("목적 1.  ", "한국어 공개 SNS 상담 경험 자료에 적용할 LLM 보조 정제 절차를 정리하고, 본문 코퍼스를 구성한다."),
               ("목적 2.  ", "본문 코퍼스의 경험 극성과 사건 언어 양상을 분포 수준에서 기술하고, 면접 기반 선행 연구와 비교한다.")], 23)
...
sp = find("이론적 배경을 작성"); setbox(sp, 55, 593, 345, 150); fill_para(sp, [iron1, iron2], 23)
...
sp = find("연구방법에 대해 작성"); setbox(sp, 55, 803, 345, 45); fill_para(sp, [bangbeop], 22)
# 전체 폭 4층 파이프라인 + 그 아래 코퍼스 퍼널 (원본 포스터와 동일 구성)
slide.shapes.add_picture(f"{FIG}/pipeline.png", Mm(56), Mm(852), width=Mm(338))
caption(56, 972, 338, "그림 1. LLM 보조 4층 정제 절차. 규칙 분류로 분량을 줄이고 경계 사례에만 LLM 판정(③)을 적용한다.")
slide.shapes.add_picture(f"{FIG}/corpus_funnel.png", Mm(128), Mm(990), width=Mm(182))
caption(128, 1095, 182, "그림 2. 원자료 3,652건 → 본문 코퍼스 361건(9.9%). 관련성 유지본 479·엄격 확정본 451건 경유.")
