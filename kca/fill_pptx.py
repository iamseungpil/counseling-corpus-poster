...
# 섹션 바·라벨을 내용에 맞게 재배치 (배경 LaTeX판과 동일 좌표)
def move_shape(name, top_mm):
    for sp in slide.shapes:
        if sp.name == name:
            sp.top = Mm(top_mm); return True
    return False
for nm, y in [("그룹 39", 390), ("Picture 11", 490), ("TextBox 12", 495),
              ("Picture 15", 705), ("TextBox 16", 708)]:
    move_shape(nm, y)
...
"직접 분석한 연구는 아직 드물다.")
sp = find("연구의 필요성을 작성"); setbox(sp, 52, 250, 345, 138); fill_para(sp, [seoron], 25)

sp = find("연구목적과 연구문제를 작성"); setbox(sp, 55, 413, 345, 72)
...
sp = find("이론적 배경을 작성"); setbox(sp, 49, 535, 345, 168); fill_para(sp, [iron1, iron2], 24)
...
sp = find("연구방법에 대해 작성"); setbox(sp, 49, 750, 345, 56); fill_para(sp, [bangbeop], 22)
# 전체 폭 4층 파이프라인 + 그 아래 코퍼스 퍼널 (원본 포스터와 동일 구성)
slide.shapes.add_picture(f"{FIG}/pipeline.png", Mm(50), Mm(814), width=Mm(340))
caption(50, 936, 340, "그림 1. LLM 보조 4층 정제 절차. 규칙 분류로 분량을 줄이고 경계 사례에만 LLM 판정(③)을 적용한다.")
slide.shapes.add_picture(f"{FIG}/corpus_funnel.png", Mm(98), Mm(960), width=Mm(238))
caption(98, 1094, 238, "그림 2. 원자료 3,652건 → 본문 코퍼스 361건(9.9%). 관련성 유지본 479·엄격 확정본 451건 경유.")
...
_tblPr.append(_sid)
tbl.columns[0].width = Mm(56); tbl.columns[1].width = Mm(140); tbl.columns[2].width = Mm(134)
