...
def center_label(lbl, bar_top_mm, dx_mm, bar_h_mm=34.3):
    ...
    sp.left = sp.left + Mm(dx_mm); sp.top = Mm(bar_top_mm - 5.5); sp.height = Mm(bar_h_mm)
    sp.width = sp.width + Mm(40)  # 대체 폰트에서도 한 줄 유지
...
set_y("그룹 39", 452); set_y("Picture 11", 550); set_y("Picture 15", 777)
center_label("TextBox 8", 179.8, L); center_label("TextBox 12", 550, L); center_label("TextBox 16", 777, L)
...
"직접 분석한 연구는 아직 드물다.")
sp = find("연구의 필요성을 작성"); setbox(sp, 58, 250, 345, 195); fill_para(sp, [seoron], 24)

sp = find("연구목적과 연구문제를 작성"); setbox(sp, 61, 478, 345, 65)
fill_para(sp, [("목적 1.  ", "한국어 공개 SNS 상담 경험 자료에 적용할 LLM 보조 정제 절차를 정리하고, 본문 코퍼스를 구성한다."),
               ("목적 2.  ", "본문 코퍼스의 경험 극성과 사건 언어 양상을 분포 수준에서 기술하고, 면접 기반 선행 연구와 비교한다.")], 24)
...
sp = find("이론적 배경을 작성"); setbox(sp, 55, 594, 345, 175); fill_para(sp, [iron1, iron2], 24)
...
sp = find("연구방법에 대해 작성"); setbox(sp, 55, 821, 345, 45); fill_para(sp, [bangbeop], 24)
# 전체 폭 4층 파이프라인 + 그 아래 코퍼스 퍼널 (원본 포스터와 동일 구성)
slide.shapes.add_picture(f"{FIG}/pipeline.png", Mm(56), Mm(868), width=Mm(338))
caption(56, 990, 338, "그림 1. LLM 보조 4층 정제 절차. 규칙 분류로 분량을 줄이고 경계 사례에만 LLM 판정(③)을 적용한다.")
slide.shapes.add_picture(f"{FIG}/corpus_funnel.png", Mm(133), Mm(1004), width=Mm(172))
caption(133, 1102, 172, "그림 2. 원자료 3,652건 → 본문 코퍼스 361건(9.9%). 관련성 유지본 479·엄격 확정본 451건 경유.")
...
fill_para(sp, [("기초 분포 — ", gi)], 23)
# overview figure
slide.shapes.add_picture(f"{FIG}/corpus_overview.png", Mm(458), Mm(290), width=Mm(318))
...
fill_para(tb, [("면접 기반 선행 연구와의 비교 — ",
  ...
  "사건 단위로 정리된 부정 경험의 표지가 익명 환경의 글에서도 적용 가능한 형태로 드러난다.")], 23)
...
fill_para(tb2, [("과정 코드의 극성별 대비 — ", gc)], 23)
tb2.text_frame.word_wrap = True
# process figure
slide.shapes.add_picture(f"{FIG}/process_contrast.png", Mm(465), Mm(672), width=Mm(300))

# ---------------- V. 논의 ----------------
sp = find("결론 및 논의를 작성"); setbox(sp, 449, 936, 338, 200)
fill_para(sp, [
  ("함의:  ", "정식 표본에서 좀처럼 함께 모이지 않던 부정과 긍정 경험이 익명 환경에서는 거의 같은 비중으로 함께 "
   "기록되었다(부정 117·긍정 126건). 사건 단위의 경험 언어도 분포 수준에서 서로 구분되었으며, 그 표지는 면접 자료를 "
